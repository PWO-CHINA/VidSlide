...
│   ├── mode-ppt.png          # 三模式示例：PPT 录屏
│   ├── mode-hybrid.png       # 三模式示例：电子课堂
│   └── mode-blackboard.png   # 三模式示例：实体课堂
...
### v0.6.1 (2026-02-28) — PyAV 加速 + 三模式视频类型

**核心新增：**
- ⚡ **PyAV 关键帧加速**：集成 PyAV 库，对 AV1 / H.264 / H.265 视频自动启用 NONKEY 关键帧迭代，提取速度提升 5-10 倍
- 🏫 **三模式视频类型**：原「实体课堂模式」升级为 PPT 录屏 / 电子课堂 / 实体课堂 三选一下拉框，适配更多课堂场景
- 🎭 **MOG2 两阶段预训练**：提取前先扫描全部关键帧建立背景模型，提取时近乎冻结学习率（0.005），防止老师被"吸收"进背景
- 🔬 **形态学遮罩增强**：9×9 闭运算填充轮廓空洞 + 13×13 膨胀扩大遮罩范围，有效过滤画面中走动的人物

**改进：**
- 🎮 **GPU 硬件解码自动检测**：CUDA → D3D11 → 回退 CPU，自动选择最优解码方式
- 🔧 **电子课堂跳过稳定帧检测**：解决快速翻页时答案页丢失的"超调"问题
- 🔧 **NONKEY 稳定帧阈值修复**：避免独立编码关键帧的微小差异导致误判
- 📖 **README 原理详解**：新增三模式工作原理说明（场景检测 / MOG2 遮罩 / 稳定帧检测 / 采样间隔），附三种典型画面截图
